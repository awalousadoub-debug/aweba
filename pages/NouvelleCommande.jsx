...
export default function NouvelleCommande({ onRetour, onSuccess }) {
  ...
  const [items, setItems]       = useState([{ product_id: '', quantity: 1, unit_price: 0, product_name: '', product_sku: '' }]);
  ...
  const updateItem = (i, field, val) => {
    ...
    if (field === 'product_id') {
      ...
      if (p) { next[i].unit_price = p.base_price; next[i].product_name = p.name; next[i].product_sku = p.sku; }
    }
    setItems(next);
  };

  const addItem    = () => setItems([...items, { product_id:'', quantity:1, unit_price:0, product_name:'', product_sku:'' }]);
  ...
  return (
    <div>
      ...
      <div style={{ background:'#fff', border:'0.5px solid #e0e0d8', borderRadius:12, padding:18, marginBottom:16 }}>
        ...
        {items.map((item, i) => (
          <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 80px 100px auto', gap:10, marginBottom:10, alignItems:'end' }}>
            <div>
              ...
              <select style={inp} value={item.product_id} onChange={e=>updateItem(i,'product_id',e.target.value)}>
                ...
                {produits.map(p => <option key={p.id} value={p.id}>{p.name} — {Number(p.base_price).toLocaleString('fr-FR')} F</option>)}
              </select>
            </div>
            <div>
              <label style={lbl}>Qté</label>
              <input style={inp} type="number" min="1" value={item.quantity} onChange={e=>updateItem(i,'quantity',parseInt(e.target.value)||1)} />
            </div>
            <div>
            ...
            <button onClick={()=>removeItem(i)} disabled={items.length===1} style={{ padding:'7px 10px', border:'0.5px solid #f0c0c0', borderRadius:8, background:'#fff', color:'#E24B4A', cursor:'pointer', fontSize:13 }}>✕</button>
          </div>
        ))}
        <button onClick={addItem} style={{ padding:'6px 14px', border:'0.5px solid #ccc', borderRadius:8, background:'#fff', cursor:'pointer', fontSize:13 }}>+ Ajouter un produit</button>
      </div>
      ...
      <div style={{ background:'#fff', border:'0.5px solid #e0e0d8', borderRadius:12, padding:18, marginBottom:16 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
          ...
          <div style={{ padding:'14px 0' }}>
            ...
            <div style={{ fontSize:12, color:'#888' }}>Livraison : {(zone?.fee||0).toLocaleString('fr-FR')} F</div>
            <div style={{ fontSize:16, fontWeight:500, marginTop:6 }}>Total : {total.toLocaleString('fr-FR')} F</div>
          </div>
        </div>
      </div>
